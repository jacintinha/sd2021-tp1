...
public interface RestSpreadsheets {
	...
	/**
	 * Creates a new spreadsheet. The sheetId and sheetURL are generated by the
	 * server. After being created, the size of the spreadsheet is not modified.
	 *
	 * @param sheet    - the spreadsheet to be created.
	 * @param password - the password of the owner of the spreadsheet.
	 * @return 200 the sheetId; 400 otherwise.
	 */
	@POST
	 ...
	 * @param password - the password of the owner of the spreadsheet.
	 * @return 204 if the sheet was successful. 404 if no sheet exists with the
	 *         given sheetId. 403 if the password is incorrect. 400 otherwise.
	 */
	@DELETE
	@Path("/{sheetId}")
	 ...
	 * @return 200 and the spreadsheet 404 if no sheet exists with the given
	 *         sheetId, or the userId does not exist. 403 if the password is
	 ...
	 * @return 200, if the operation is successful 403, if the spreadsheet is not
	 *         shared with user, or the user is not the owner, or the password is
	 *         incorrect. 404, if the spreadsheet or the user do not exist 400,
	 *         otherwise
	 */
	@GET
	@Path("/{sheetId}/values")
	@Produces(MediaType.APPLICATION_JSON)
	List<List<String>> getSpreadsheetValues(@PathParam("sheetId") String sheetId, @QueryParam("userId") String userId,
			@QueryParam("password") String password);
	 ...
	 * @param password - the password of the owner of the spreadsheet
	 * @return 204, if the operation was successful 404, if no spreadsheet exists
	 *         with the given sheetid 403, if the password is incorrect. 400
	 *         otherwise
	 **/
	@PUT
	@Path("/{sheetId}/{cell}")
	...
	/**
	 * Adds a new user to the list of shares of a spreadsheet. Only the owner can
	 * call this method.
	 *
	 * @param sheetId  - the sheet being shared.
	 * @param userId   - the user that is being added to the list of shares. In this
	 *                 method, the userId is represented in the form userId@domain
	 * @param password - The password of the owner of the spreadsheet.
	 * @return 204, in case of success. 404, if either the spreadsheet or user do
	 *         not exist 409, if the sheet is already shared with the user 403 if
	...
	/**
	 * Removes a user from the list of shares of a spreadsheet. Only the owner can
	 * call this method.
	 *
	 * @param sheetId  - the sheet being shared.
	 * @param userId   - the user that is being added to the list of shares. In this
	 *                 method, the userId is represented in the form userId@domain
	 * @param password - The password of the owner of the spreadsheet.
	 * @return 204, in case of success. 404, if the spreadsheet, the user or the
	 *         share do not exist 403 if the password is incorrect. 400, otherwise
	 */
	@DELETE
	@Path("/{sheetId}/share/{userId}")
